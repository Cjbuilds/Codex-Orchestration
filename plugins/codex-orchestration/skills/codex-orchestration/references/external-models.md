...
2. A bundled, reviewed first-party subscription CLI adapter. Claude Fable 5 and
   the exact Claude Opus 5 route retain the no-tools, no-session-persistence,
   first-party-login, runtime-metadata bridge. Opus may additionally serve the
   persistent Designer through the read-only `create_design` operation; Fable is
