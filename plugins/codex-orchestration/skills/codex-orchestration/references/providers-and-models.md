...
Claude Fable is the explicit built-in exception for Planner or Advisor. The plugin does not pretend it is a Codex model or translate Anthropic into the Responses protocol. Instead, a disabled-by-default local MCP server invokes the official `claude` CLI with either the user's first-party Pro or Max login or optional managed `ANTHROPIC_*` child-env overrides. The seat primary defaults to `claude-fable-5` and may be customized with setup flags. Setup enables one Python 3.11+ launcher variant, and disable restores every prior plugin override value. Codex's TOML editor can retain an inert empty table header after its final key is deleted; the configurator does not risk a broad TOML rewrite for cosmetic cleanup.

The bridge removes parent-process `ANTHROPIC_API_KEY`, `ANTHROPIC_AUTH_TOKEN`, `ANTHROPIC_BASE_URL`, and Bedrock/Vertex/Foundry selection variables from the child environment, then injects only the optional managed pair from routing state (`claude_env.ANTHROPIC_BASE_URL` / `claude_env.ANTHROPIC_AUTH_TOKEN`) when setup saved them. Auth is either first-party Pro/Max (`claude auth status`) or the managed token path. It pins the seat-saved Claude Code model (default `claude-fable-5`, or a custom `--planner-fable-model` / `--advisor-fable-model`) and the saved effort, disables tools and session persistence, disables prompt suggestions, and requires JSON runtime metadata to contain that primary model. Claude Code currently reports the internal helper `claude-haiku-4-5-20251001` during valid Fable calls; the bridge permits only that exact helper ID and returns every observed ID in `used_models`. Missing Fable or any unknown additional model fails closed. Helper rotation therefore requires a reviewed plugin update rather than a wildcard. Setup and status never make a model call.
