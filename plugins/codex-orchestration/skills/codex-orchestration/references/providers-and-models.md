...
| `usage_hint_text` | Appended to the spawn tool description | Carries the exact Planner/Advisor/Designer/Executor routes where the root chooses children. |
...
| Claude Opus 5 MCP route | Uses the same bounded compatibility launchers with exact model `claude-opus-5` and Claude Code 2.1.219+ | Sealed Planner, Advisor, or persistent Designer; Designer may share one exact Opus subscription identity with one planning seat. |
