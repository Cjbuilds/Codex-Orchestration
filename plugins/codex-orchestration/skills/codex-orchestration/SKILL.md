...
Normalize `Extra High` to `xhigh`. For Claude Fable, accept `Low`, `Medium`, `High`, `XHigh`, `Max`, or `Ultra`. Omission or `Auto` means `High`; `Ultra` is a user-facing alias for Claude Code's actual `max` setting and must be reported as that mapping. Route Fable with `--planner-fable --planner-effort <normalized-effort>` or `--advisor-fable --advisor-effort <normalized-effort>`, not through the Codex model catalog. Optionally pin the Claude Code primary with `--planner-fable-model <id>` or `--advisor-fable-model <id>`; omission defaults to `claude-fable-5`. Resolve every other display name to an exact ID only through the executing host's model catalog, picker, a loaded custom agent, or official provider documentation. Never invent an ID. For persistent direct routing, resolve `auto` to the catalog's concrete default.
...
Add `--advisor-model` and `--advisor-effort` for a same-provider Codex advisor. For Claude Fable, use `--advisor-fable`; add `--advisor-effort low|medium|high|xhigh|max` when the user chooses one. Optional `--advisor-fable-model <claude-code-model-id>` pins the actual Claude Code model for that seat; omission defaults to `claude-fable-5`. Omitting Fable effort defaults to `high`, while user-facing `ultra` is normalized to Claude Code's `max`. The configurator verifies that the installed Claude Code CLI advertises the selected effective effort. It also requires Claude Code to be logged in through a first-party Pro or Max account **or** an optional managed token path, chooses an available Python 3.11+ MCP launcher, and performs only an auth/capability check during setup. It never extracts a token from Claude login and makes no model call during setup or status. Optional `--anthropic-base-url` and `--anthropic-auth-token` may persist Claude child env overrides into the 0o600 routing state for Fable forks only; use `--clear-claude-env` to drop them. Parent-process `ANTHROPIC_*` values are not trusted. Omission persists `advisor: none`.

Add `--planner-model` and `--planner-effort` for a same-provider Planner. For Claude Fable, use `--planner-fable`; add `--planner-effort low|medium|high|xhigh|max` when the user chooses one. Optional `--planner-fable-model <claude-code-model-id>` pins the actual Claude Code model for that seat; omission defaults to `claude-fable-5`. Planner omission persists no Planner route and means the root plans. A configured Planner and Advisor must not resolve to the same model or agent route; independent review is required.
...
## Claude Fable Planner or Advisor

Use this built-in route when the user names Claude Fable (including the default product label Claude Fable 5). Do not create a custom provider or custom-agent file for it.

In user-facing status or results, report the default product path as `Claude Fable 5`. When setup pinned a custom Claude Code primary via `--planner-fable-model` or `--advisor-fable-model`, report that exact model ID instead of forcing the Fable 5 label. Report authentication as `first-party login ready` or `env token` as appropriate; do not expose or restate Claude account-plan metadata or token values.

Prerequisites:

- the official `claude` CLI is installed;
- either `claude auth status` reports a first-party Pro or Max login, **or** setup saved `ANTHROPIC_AUTH_TOKEN` (optionally with `ANTHROPIC_BASE_URL`) for the Fable child env;
- a Python 3.11+ launcher is available.

The plugin packages three disabled MCP launcher variants for macOS, Linux, and Windows. Setup enables exactly one compatible variant through the plugin's namespaced config when either Fable seat is selected. At planning or review time the MCP server strips parent-process API-key / token / Base URL / Bedrock / Vertex / Foundry override variables, reloads any managed Claude env from routing state into the forked `claude` process only, re-checks auth (first-party login or env token), and invokes `claude -p --model <seat-pinned-primary>` with `--safe-mode`, no tools, no session persistence, prompt suggestions disabled, and JSON output. The seat-pinned primary defaults to `claude-fable-5` unless setup supplied `--planner-fable-model` or `--advisor-fable-model`. Each saved seat pins its model and effort; the root cannot replace them through tool arguments.
