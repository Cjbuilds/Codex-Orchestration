...
9. Upgrade from the previous public version in a clean Codex home, reinstall the plugin, and verify the installed version and skill contents changed before starting a new task. Then verify setup, `status --require-effective`, and disable. For schema 7 or later, run an isolated alternate-marketplace lifecycle with a disabled canonical sibling and prove setup targets the enabled executing identity while disable restores the saved identity even after it becomes disabled.
