...
| Medium | Planner support reused the same `0.5.0` identity as an earlier Advisor-only bundle, allowing a stale marketplace/cache to keep role-obsolete instructions. | Release the complete Planner contract as `0.5.2`, treat explicit seat labels as authoritative, and lifecycle-test upgrade from the exact affected bundle into an installed Planner-capable package. |
...
| Medium | Released version `0.4.0` had no immutable tag or GitHub release. | Add a tag/version release gate; `0.5.2` must ship from a signed `v0.5.2` tag and matching GitHub release. |
