...
- Kimi K3 Designer is a second narrow built-in exception: it uses the authenticated first-party Kimi Code CLI through a fresh ACP session with permissions denied, terminal disabled, no MCP servers, an empty temporary cwd, and transcript rejection for tool or filesystem activity. The bridge verifies the ACP-selected K3 model, requires catalog support for `max`, and injects the documented `KIMI_MODEL_THINKING_EFFORT=max` wire override because ACP does not expose a separate effort identity.
- Qwen 3.8 Max Preview Advisor is a third narrow built-in exception: it uses official Qwen Code 0.20.0, an OS-stored regional Token Plan credential, one allowlisted endpoint, no custom context or chat recording, an explicit denylist for every remaining core tool plus a zero-call budget, an empty temporary cwd, and fail-closed structured runtime/model validation. The key exists briefly as an immutable child-environment string and cannot be securely erased in-process.
...
- The cross-provider storage systems cannot be committed atomically by the current public interfaces. Status and bounded managed-role cleanup provide recovery without deleting edited or user-owned files.
